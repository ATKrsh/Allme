...
def kill_previous_instances():
    ...
    if is_windows:
        ...
        try:
            subprocess.run(['pkill', '-f', 'Allme'], capture_output=True)
            subprocess.run(['pkill', '-f', 'AllMeD'], capture_output=True)
        except Exception: pass
...
DEFAULT_CONFIG = {
    ...
    "telemetry_source": "cpu_usage",# "cpu_usage", "cpu_freq", "hdd", "memory", "ethernet", "gpu", "power", "mouse", "keyboard", "breathing", "none"
    "telemetry_target": "app_size", # "app_size", "app_color", "glow_size", "glow_color", "app_opacity", "glow_opacity", "app_glow_size", "app_glow_color", "app_glow_opacity", "none"
    "hdd_drive": "All",
    "hdd_mode": "both",             # "both", "read", "write"
    "net_mode": "both",             # "both", "upload", "download", "ping"
    "gpu_choice": "gpu0",           # "gpu0", "gpu1", "max"
...
class FaviconManager:
    ...
    def _get_browser_url(self, hwnd):
        if not is_windows:
            # macOS browser URL extraction via AppleScript would go here, 
            # for now, return None to fallback to generic icon
            return None
        try:
            ps = (
...
def get_window_title(hwnd):
    if not is_windows: return "macOS App"
...
class DashboardWindow(QDialog):
    def __init__(self, main_app):
        ...
        # --- TELEMETRY ---
        v2 = QVBoxLayout()
        add_combo(v2, "Link Source", [
            ("0. None", "none"), ("1. Breathing", "breathing"), ("2. CPU Freq", "cpu_freq"), ("3. CPU Usage", "cpu_usage"),
            ("4. HDD", "hdd"), ("5. Memory", "memory"), ("6. Ethernet", "ethernet"), ("7. GPU", "gpu"),
            ("8. Power", "power"), ("9. Keyboard", "keyboard")
        ], main_app.config.get("telemetry_source", "cpu_usage"), main_app._set_telemetry_source)
        add_combo(v2, "Link Sink", [
        ...
        grp2 = QGroupBox("Telemetry Links"); grp2.setLayout(v2); grid.addWidget(grp2, 0, 1)
...
class AllmeWidget(QWidget):
    ...
    def _setup_tray(self):
        ...
        # 📊 Submenu 4: Renamed to "Link Source" (10 Telemetry Channels)
        source_menu = self._create_sub_menu(self.tray_menu, "📊 Link Source")
        cur_src = self.config.get("telemetry_source", "breathing")
        src_grp = QActionGroup(self); src_grp.setExclusive(True)

        src_items = [
            ("0. None", "none"),
            ("1. Normal Human Breathing Sine", "breathing"),
            ("2. CPU Frequency", "cpu_freq"),
            ("3. CPU Usage", "cpu_usage"),
            ("4. HDD Activity", "hdd"),
            ("5. Memory Usage (Sensitive)", "memory"),
            ("6. Ethernet & Ping", "ethernet"),
            ("7. GPU Usage", "gpu"),
            ("8. System Power Usage", "power"),
            ("9. Keyboard Typing Speed (WPM)", "keyboard")
        ]
        for label, code in src_items:
            act = QAction(label, self)
            act.setCheckable(True)
        ...
        # HDD Submenu (Drives & IO mode)
        source_menu.addSeparator()
        hdd_sub = self._create_sub_menu(source_menu, "📁 HDD Settings")
        cur_drv = self.config.get("hdd_drive", "All")
        drv_grp = QActionGroup(self); drv_grp.setExclusive(True)
        for drv in ["All", "PhysicalDrive0", "PhysicalDrive1", "PhysicalDrive2"]:
            act = QAction(f"Drive: {drv}", self)
            act.setCheckable(True)
            drv_grp.addAction(act)
            if cur_drv == drv: act.setChecked(True)
            act.triggered.connect(lambda _c, d=drv: self._set_config_val("hdd_drive", d))
            hdd_sub.addAction(act)
        hdd_sub.addSeparator()
        cur_io = self.config.get("hdd_mode", "both")
        io_grp = QActionGroup(self); io_grp.setExclusive(True)
        for mode_lbl, mode_code in [("IO: Both (Read+Write)", "both"), ("IO: Read Only", "read"), ("IO: Write Only", "write")]:
            act = QAction(mode_lbl, self)
            act.setCheckable(True)
            io_grp.addAction(act)
            if cur_io == mode_code: act.setChecked(True)
            act.triggered.connect(lambda _c, m=mode_code: self._set_config_val("hdd_mode", m))
            hdd_sub.addAction(act)

        # Ethernet Submenu
        net_sub = self._create_sub_menu(source_menu, "🌐 Ethernet / Ping Settings")
        cur_net = self.config.get("net_mode", "both")
        net_grp = QActionGroup(self); net_grp.setExclusive(True)
        for mode_lbl, mode_code in [("Net: Both (Upload+Download)", "both"), ("Net: Upload Only", "upload"), ("Net: Download Only", "download"), ("Net: Ping Latency (ms)", "ping")]:
            act = QAction(mode_lbl, self)
            act.setCheckable(True)
            net_grp.addAction(act)
            if cur_net == mode_code: act.setChecked(True)
            act.triggered.connect(lambda _c, m=mode_code: self._set_config_val("net_mode", m))
            net_sub.addAction(act)

        # GPU Submenu
        gpu_sub = self._create_sub_menu(source_menu, "🎮 GPU Settings")
        cur_gpu = self.config.get("gpu_choice", "gpu0")
        gpu_grp = QActionGroup(self); gpu_grp.setExclusive(True)
        for mode_lbl, mode_code in [("GPU 0 (Primary)", "gpu0"), ("GPU 1 (Secondary)", "gpu1"), ("Max GPU", "max")]:
            act = QAction(mode_lbl, self)
            act.setCheckable(True)
            gpu_grp.addAction(act)
            if cur_gpu == mode_code: act.setChecked(True)
            act.triggered.connect(lambda _c, m=mode_code: self._set_config_val("gpu_choice", m))
            gpu_sub.addAction(act)

        # 🔗 Submenu 5: Renamed to "Link Sink" (9 Targets including App+Glow)
        ...
        tgt_items = [
            ("0. None", "none"),
            ("1. App Size", "app_size"),
            ("2. App Color (Red -> Blue Shift)", "app_color"),
            ("3. Glow Size", "glow_size"),
            ("4. Glow Color (Blue -> Red Shift)", "glow_color"),
            ("5. App Opacity", "app_opacity"),
            ("6. Glow Opacity", "glow_opacity"),
            ("7. App + Glow Size", "app_glow_size"),
            ("8. App + Glow Color Shift", "app_glow_color"),
            ("9. App + Glow Opacity", "app_glow_opacity")
        ]
        for label, code in tgt_items:
            act = QAction(label, self)
            act.setCheckable(True)
